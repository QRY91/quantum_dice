...
signal boon_activated(boon_id: String, boon_display_name: String, boon_description: String)
...
# --- State Variables ---
var active_boons: Dictionary = {} # Key: boon_id (String), Value: true (or more detailed boon data if needed)
...
func check_and_activate_rune_phrases(current_runes_in_history_ids: Array[String]) -> Array[Dictionary]: # Returns array of activated boon info
	...
	for phrase_key in RUNE_PHRASES:
		var phrase_data: Dictionary = RUNE_PHRASES[phrase_key]
		var phrase_id: String = phrase_data.id
		
		if not active_boons.has(phrase_id): # Only check if not already active
			...
			if all_required_runes_found:
				...
				active_boons[phrase_id] = true # Mark as active
				_apply_boon_effect(phrase_id)   # Apply its immediate/ongoing effects
				
				newly_activated_boon_messages.append({
					"name": phrase_data.display_name,
					"description": phrase_data.boon_description
				})
				emit_signal("boon_activated", phrase_id, phrase_data.display_name, phrase_data.boon_description)
...
func _apply_boon_effect(boon_id: String):
	...
	match boon_id:
		...
		_:
			printerr("BoonManager: Attempted to apply unknown boon effect for id: ", boon_id)

# --- Getters for other systems to query boon effects ---
func get_extra_points_for_dice_glyph() -> int:
	return extra_points_per_dice_glyph if active_boons.has("sun_power") else 0 # Ensure boon is active

func get_extra_max_rolls() -> int:
	return extra_max_rolls # This value accumulates
